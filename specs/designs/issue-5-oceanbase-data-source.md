...
| R9 | **Oracle 模式 Python 驱动存在硬冲突**（本项为当前头号风险） | 可能无可行驱动，或需为适配服务单列 Python 运行时 | 仓库 `requires-python = ">=3.12,<3.14"`；OceanBase 官方文档对 Oracle 模式给出的是 **cx_Oracle + libobclient/OBCI**（需 `LD_LIBRARY_PATH`），而 **cx_Oracle 8.3.0 是末版、不支持 Python 3.12+**；`python-oracledb` **thin 模式**（纯 Python、无需客户端库）与 OB Oracle 模式的兼容性**未见证实**。候选路径见 §11.2，须 spike 后定夺 |
| R10 | Oracle 模式缺 MySQL 式的 `MAX_EXECUTION_TIME` 语句级超时 | A07「数据库端查询必须停止」可能更难满足 | 需在目标实例验证 `oracledb`/OCIBreak 类中断（`connection.cancel()`）或会话终止手段是否真能停下服务端查询；不达标则不进稳定版验收 |
| R11 | Oracle 模式下标识符大小写与引用语义（未加引号即大写） | 元数据与查询对不上对象 | 遵循需求 §4「保留大小写与引用语义」；`RTD6` 等按大写原样处理 |
...
### 11.1 已提供的环境事实（2026-09-14）

| 项 | 值 |
|---|---|
| OB 版本 | **4.3.5.6** |
| 租户 / 兼容模式 | **Oracle 兼容模式**（非 MySQL 模式） |
| 连接方式 | 经 **OBProxy**（端口 2883）——检测结论「疑似 OBProxy，可能不准确」，待确认 |
| OB 地址 | `10.88.8.29:2883` |
| 服务名 | `rtd6` |
| 完整用户名 | `askquery@rptdb#hldw`（OBProxy 形式，**原样保存**，符合 D12） |
| 实际用户名 / 当前 Schema | `ASKQUERY` / `RTD6` |
| 数据库（租户） | `rptdb` |
| TLS | **不要求**（印证 D10：TLS 必须按源可配，不能沿用现有子系统的 MySQL TLS 强制） |
| 适配服务运行位置 | docker host `aysap01` = `10.250.11.201`，容器 `code_code_sandbox`（`172.18.0.2:8080`）映射到宿主 `8989`，容器内为 FastAPI 应用；该容器可访问 `10.88.8.29:2883` |

**凭据管理**：数据库口令**不记录在本文件**，也不得进入任何提交物。按需求 OB01 走既有凭据机制（`EncryptedText` + `AUTH_ENCRYPTION_SECRET`，配置只保存 `credential_ref`）。注意：现有沙箱应用采用 `datasources.yaml` 明文口令的做法**不可移植**到本仓库——仓库启用了 detect-secrets 门禁（`.secrets.baseline`）且需求明令口令不得出现在配置查询、导出、日志与工具描述中。

### 11.2 驱动候选路径（R9 的解法，须 spike 定夺）

| 路径 | 做法 | 优点 | 风险 |
|---|---|---|---|
| **A** | `python-oracledb` **Thick 模式** + OB 的 `libobclient`/`OBCI` | 保持仓库 Python 3.12+ 单一运行时；驱动仍在维护；官方迁移文档称 Thick 模式与 cx_Oracle 8.3 功能等价 | OB Oracle 模式的兼容性**未获证实**；容器需携带 OBCI 客户端库并设 `LD_LIBRARY_PATH` |
| **B** | 适配服务单列旧版 Python + `cx_Oracle 8.3.0` + `libobclient`/`OBCI` | 与现有沙箱应用**同栈**，已在该环境实证可用；适配服务本就是独立容器（D1），隔离成本低 | 仓库内出现第二套 Python 运行时；驱动已停止演进 |
| **C** | Java 适配器 + OceanBase Connector/J | 官方列明支持 MySQL 与 Oracle **两种模式**（需求 §9 已列为备选） | 引入全新技术栈，与仓库现状（Python/Rust）差异大 |

选择依据应由 spike 的实测结果决定：**能否连上、参数绑定是否可用、超时与取消是否真能停住服务端查询、类型是否保真**。任一模式无法证明终止能力的，不进入稳定版验收（需求 §5）。

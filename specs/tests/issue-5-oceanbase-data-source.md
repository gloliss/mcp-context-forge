...
| 探测/声明/验证（D11） | 探测值 ≠ 声明值时返回明确错误；声明值 ∉ `validated_compat_modes` 时拒绝连接与发布（**构造「探测到某模式」但未验证的用例，断言仍被拒绝**）；探测失败显示「未验证」而非伪造成功 | A01 A05 |
| TLS 与降级防护（D10） | `tls_config` 按源生效（启用/不启用各一例）；**断言连接失败后不发生明文回落**，也不静默换端口或用户名格式重试 | A11 |
| 连接入口（D12） | 用户名**原样保存**（含 OBProxy 形式的 `user@tenant#cluster` 不被改写）；测试连接由适配服务侧发起而非浏览器路径 | A01 |

...
### 未验证范围（逐项精确，2026-09-18）

| 范围 | 状态 |
|---|---|
| 18 项检查（M1–M9 / O1–O9）在 **OceanBase** 上 | **零执行**。执行环境不可达 OceanBase，且 MySQL 模式没有目标实例 |
| **Oracle O1–O9 在任何真实服务器上** | **零执行**。MariaDB 自检后端说的是 MySQL 协议，覆盖不到 Oracle |
| Oracle 各检查的**代码路径**被驱动的情况 | 仅 `O3`、`O6` 由脚本化游标覆盖（`tests/test_query_timeout_check.py`、`tests/test_bind_checks.py`）；`O1`/`O2`/`O4`/`O5`/`O7`/`O8`/`O9` 只有 L1 纯函数测试，**函数体本身从未执行过** |
| MySQL M1–M9 的代码路径 | MariaDB 自检后端全部真跑过（M6 预期失败，且失败原因正确） |

两点结论：

1. **这 18 项没有一项在 OceanBase 上执行过**，本文档所有「某驱动在 OB 上可用」的结论一律为空。
2. **Oracle 侧更弱一层**：它连「代码至少被真实服务器驱动过」都没有。本轮复查中修掉的 4 处缺陷里
   有 2 处正是靠脚本化游标才发现的，而同样的手法目前只铺到了 O3/O6。

**代码质量层面**已做过系统复查（本轮查出并修掉 4 处缺陷，其中 2 处会产生错误的通过）；
**结论层面**仍然是空的。这两件事不是一回事，不要互相替代。
